...
def extract_log_any(text) -> str | None:
    """
    Extract 4-6 digit log from any string forms:
      - "Agent 7353 ..."
      - "7353 - NAME"
      - "ID: 7353"
      - "Log 7353"
    ...
    # prefer patterns with keywords
    m = re.search(r"(?:agent|log|id)\D*(\d{4,6})", t, flags=re.IGNORECASE)
    if m:
        return m.group(1)

    # fallback: first 4-6 digit group
...
def code1_clean_raw(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # best-effort rename if Teleopti/Export has Unnamed columns
    rename_map = {}
    ...
    # keep only needed
    # (do not hard-drop unknown columns; just ensure these exist)
    ...
    # remove "en attente" special line if present
    df = df[df["Etat"].astype(str).str.lower() != "en attente"]

    # convert "1h02'03" if needed
...
def code3_merge_compo(df_compo: pd.DataFrame, df_rapport: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    # filter non-empty agent names if exists
...
def code5_add_tl_sheet(excel_agent_bytes: bytes) -> bytes:
    ...
    if "Productivité" in df.columns:
        df["Productivité"] = pd.to_numeric(df["Productivité"], errors="coerce")

    df["Taux Post-travail"] = np.where(
        df.get("Temps total Travail", 0).fillna(0) > 0,
        df.get("Post-travail - Temps total", np.nan) / df.get("Temps total Travail", np.nan),
        np.nan
    )
    df["Taux d'occupation"] = np.where(
        df.get("Temps Total présence", 0).fillna(0) > 0,
        df.get("Temps total Travail", np.nan) / df.get("Temps Total présence", np.nan),
        np.nan
    )

    cols = ["Productivité", "Taux Post-travail", "DMC", "DMT", "Taux d'occupation"]
    cols = [c for c in cols if c in df.columns]

    df_tl = df.groupby("Tls")[cols].mean().reset_index() if "Tls" in df.columns else pd.DataFrame(columns=["Tls"] + cols)

    # keep TL even if productivity NaN/0 -> do not drop all
    # but remove lines fully empty
    if cols:
        df_tl = df_tl[~df_tl[cols].isna().all(axis=1)].copy()

    # TOTAL
    total_vals = [df_tl[c].mean() if len(df_tl) else np.nan for c in cols]
    total = pd.DataFrame([["TOTAL"] + total_vals], columns=["Tls"] + cols)
    df_tl = pd.concat([df_tl, total], ignore_index=True)

    # DMC/DMT to timedelta for display
    for c in ["DMC", "DMT"]:
    ...
    # CF only if we have at least 1 TL + TOTAL
    last_row = ws.max_row
    if last_row >= 3:
        end_row = last_row - 1  # exclude TOTAL
...
# =========================
# CODE 6: Email (affiche "-" si pas de data)
# =========================
def code6_build_email(excel_final_bytes: bytes, projet_upper: str, date_txt: str, signature_name: str):
    ...
    subj_occ_txt = kpi_or_dash_pct(kpi_occ, 1) if kpi_occ is not None else "-"
    ...
    def fmt_cell(c, v):
        if pd.isna(v):
            return "-"
        if c in ("Taux Post-travail", "Taux d'occupation"):
    ...
    <tr style="background:#203864;color:white;">
      <th>TL</th><th>Productivité</th><th>Taux Post-travail</th><th>DMC</th><th>DMT</th><th>Taux d’occupation</th>
    </tr>
    </thead><tbody>
    """
...
def build_flop_from_excel(excel_final_bytes: bytes, kpi_choice: str, flop_n: int, direction: str) -> pd.DataFrame:
    ...
    # remove <10min presence (si possible)
